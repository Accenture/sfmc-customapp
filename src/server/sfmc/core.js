...
// 			: body.OverallStatus;
// 		this.errors = body.Results ? body.Results.map((e, i) => {
// 			return {
// 				errorNumber: i,
...
// export function soapRequest = async (body, action, auth) => {
// 	const envelope = {
// 		Body: body,
// 		$: {
...
// 			message:
// 				json["soap:Envelope"]["soap:Body"][0]["soap:Fault"][0].faultstring[0]
// 		});
// 	}
// 	throw new SoapError({
...
// export function getToken = async (req) => {
...
// 	}
// 	await new Promise((resolve, reject) => {
// 		req.sessionStore.set(req.query.state, currentSession, (err, session) => {
// 			if (err) reject(err);
// 			resolve(session);
// 		});
// 	});
...
// export function getContext = async (auth) => {
...
// 	return res.data;
// };
// export function getRedirectURL = (req, appname) => {
...
// 	/*needed for testing */
// 	req.session.auth.redirect_uri = process.env.NODE_ENV === "production" ? `https://${req.hostname}/sfmc/auth/response` : `https://${req.hostname}:${process.env.PORT}/sfmc/auth/response`;
...
// export function refreshToken = async (auth) => {
// 	const response = await axios({
// 		method: "post",
...
// 	try {
// 		if (req.session && req.session.auth && req.session.auth.expiryDateTime) {
...
// export function authenicate = async (req, res) => {
// 	if (req.query && req.query.error && req.query.error === "user_not_licensed") {
...
// export function parseSOAPResponse = (body) => {
// 	if (body.OverallStatus[0] === "OK") {
// 		return body.Results;
// 	}
// 	throw new SoapError(body);
// };
// export function restproxy = async (req) => {
// 	const response = await axios({
// 		method: req.method,
